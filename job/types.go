...
// Manager is the interface to used to represent all of the use cases which
...
type Manager interface {
	...
	Create(*Job) error
	Execute(*Job) error
	GetLog(*Job, int) (*Log, error)
	Delete(*Job) error
}

// Repository is the interface that wraps all of the persistence operations
// related to a job. The Manager uses the Repository to maintain state
...
type Repository interface {
	...
	GetJobLog(jobID string, index int) (*Log, error)
	AppendLogLine(jobID, logLine string) error
}

// StepExecutor is the interface that wraps the methods necessary to turn
...
// CurrentStep returns the first Step from the list which has not yet
...
// Step represents one of the individual steps in a Dray Job. A job step is
...
func (js Step) usesStdOutPipe() bool {
	return js.Output == "stdout" || js.Output == ""
}

func (js Step) usesStdErrPipe() bool {
	return js.Output == "stderr"
}

func (js Step) usesFilePipe() bool {
	return strings.HasPrefix(js.Output, "/")
}

func (js Step) filePipePath() string {
	return fmt.Sprintf("/tmp/%x", md5.Sum([]byte(js.Source))) // nolint:gas
}

func (js Step) usesDelimitedOutput() bool {
	return len(js.BeginDelimiter) > 0 && len(js.EndDelimiter) > 0
}

// Log represents the log output of a job. The Index field contains the
